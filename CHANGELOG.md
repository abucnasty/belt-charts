...
### Fixed
- `line`/`bar` commands no longer run out of memory on large batches of CSVs. CSV parsing is now parallelized across a bounded worker pool while chart rendering stays single-threaded on the main thread, and only the requested `--metrics` are parsed/transferred instead of every column in the file.
